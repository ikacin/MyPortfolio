...
Settings → Pages → Build and deployment → Source: “Deploy from a branch” seçin, Branch: gh-pages (klasör: / (root)). main seçiliyse sayfa beyaz kalır ve /src/main.jsx 404 hatası alırsınız — build çıktısı sadece gh-pages branch’inde.
main branch’e her push’ta workflow build alır ve gh-pages branch’ine yükler; Pages bu branch’ten yayınlar.
Site: https://<kullanici>.github.io/<repo-adı>/ (örn. MyPortfolio repo ise: .../MyPortfolio/)
...
GitHub’da repoya gidin → Settings → Pages.
...
Kaydedin. Birkaç dakika bekleyin.
Actions sekmesinde “Deploy to GitHub Pages” workflow’unun yeşil (başarılı) çalıştığından emin olun. Başarısızsa hata mesajına göre düzeltin.
Tarayıcıda sert yenileme yapın (Ctrl+Shift+R / Cmd+Shift+R) veya gizli pencerede açın.
